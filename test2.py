...
def load_and_preprocess_data(csv_path, seq_length=SEQ_LENGTH, total_balls=TOTAL_BALLS):
...
def train_momentum_model(csv_path, seq_length=SEQ_LENGTH, epochs=10, batch_size=32, total_balls=TOTAL_BALLS):
...
def generate_live_report(live_csv, historical_csv, seq_length, ball_features, cum_features, ctx_features, chase_features, ball_scaler, cum_scaler, ctx_scaler, chase_scaler, le):
...
if __name__ == "__main__":
    ...
    live_csv = "over5.csv"                     # Live match segment CSV (e.g., first 5 overs / 30 balls)
